...
# model pulse arrays
mod1 = mcr.model_1exp(1, 1e-2, 5e-3, t0=0.1)(time)
mod2 = mcr.model_2exp(0.9, 0.1, 1e-2, 1e-1, 5e-3, t0=0.4)(time, details=True)
mod3 = mcr.model_3exp(0.7, 0.2, 0.1, 1e-2, 5e-2, 1e-1, 5e-3, t0=0.7)(
    time, details=True
)
...
noise_psd = mcr.dim_psd(noise_fft, t_step, t_len)
noise_welch = sgl.welch(noise, fs, window='boxcar', nperseg=ns)[1][1:]
noise_angle = np.angle(noise_fft)

tru_lab = 'true fft'
tru_fft = mcr.model_1exp_fft(ns, *param, t0=0.1)(freq)
tru_psd = mcr.dim_psd(tru_fft, t_step, t_len)
#    tru_pulse = np.fft.ifft(model_1exp_fft(*param, t0=0.1)(np.fft.fftfreq(ns, fs**-1)) * ns)
tru_pulse = mcr.dim_ifft(mcr.model_1exp_fft(ns, *param, t0=0.1), fs, ns)
